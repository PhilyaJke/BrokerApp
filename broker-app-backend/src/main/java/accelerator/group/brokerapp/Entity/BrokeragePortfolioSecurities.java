...
@NamedNativeQueries({
        @NamedNativeQuery(
                ...
                query = "SELECT bps.id, bps.count FROM brokerage_portfolio_securities bps INNER JOIN brokerage_portfolio bp on bp.id = bps.brokerage_portfolio_id" +
        ...
        @NamedNativeQuery(
                ...
                query = "SELECT s.id, s.region, asi.price, s.name, s.ticker, s.sector, s.icon_path AS iconPath FROM securities s" +
...
public class BrokeragePortfolioSecurities {
    ...
    public BrokeragePortfolioSecurities(long id, long count) {
        this.id = id;
        this.count = count;
    }
